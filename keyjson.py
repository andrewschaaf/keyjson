...
def dumps(x, allowStructures=True):
    ...
    if allowStructures:
        ...
        if isinstance(x, list) or isinstance(x, tuple):
            return LIST + '\x00'.join(dumps(elem, allowStructures=False) for elem in x)
        
        if isinstance(x, dict):
            keys = [(dumps(k, allowStructures=False), k) for k in x.keys()]
            arr = []
            for k_keyjson, k in sorted(keys):
                arr.append(k)
                arr.append(x[k])
            return DICT + '\x00'.join(dumps(elem, allowStructures=False) for elem in arr)
...
def loads(data):
    ...
    if prefix == LIST:
        return [loads(chunk) for chunk in data[1:].split('\x00')]
    
    if prefix == DICT:
        arr = [loads(chunk) for chunk in data[1:].split('\x00')]
